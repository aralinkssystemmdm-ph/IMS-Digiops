...
const DeliveryReceiptManagement: React.FC<DeliveryReceiptManagementProps> = ({ isDarkMode = false }) => {
  ...
  const confirmDeleteDR = async () => {
    ...
    if (isSupabaseConfigured) {
      try {
        ...
        console.warn('Failed to delete delivery receipt from Supabase:', err);
      }
    }
  ...
  const handleChangeStatus = (drId: string, newStatus: DeliveryReceipt['status']) => {
    const receiptObj = receipts.find(r => r.id === drId);
    if (!receiptObj) return;

    // If newStatus is the same as current status, do nothing
    if (newStatus === receiptObj.status) return;

    // State Transition Constraints:
    // 1. Delivered = no action
    if (receiptObj.status === 'Delivered') {
      showWarning('Action Restricted', 'Delivered records are locked and cannot be changed.');
      return;
    }

    // 2. Ready for Delivery => In Transit only
    if (receiptObj.status === 'Ready for delivery' && newStatus !== 'In Transit') {
      showWarning('Action Restricted', 'Ready for Delivery status can only transition to In Transit.');
      return;
    }

    // 3. In Transit/Partially Delivered => Delivered only
    if ((receiptObj.status === 'In Transit' || receiptObj.status === 'Partially Delivered') && newStatus !== 'Delivered' && newStatus !== 'Partially Delivered') {
      showWarning('Action Restricted', 'In Transit status can only transition to Delivered.');
      return;
    }

    if (newStatus === 'Ready for delivery') {
      if (receiptObj.status === 'In Transit' || receiptObj.status === 'Delivered' || receiptObj.status === 'Partially Delivered') {
  ...
  const handleConfirmItemValidation = () => {
  ...
  const handleConfirmStatusDate = () => {
    ...
    if (selectedDR?.id === drId) {
      setSelectedDR(prev => {
        if (!prev) return null;
        if (newStatus === 'In Transit') {
          return { ...prev, status: newStatus, inTransitDate: dateVal, deliveredDate: undefined };
        } else {
  ...
  const statistics = useMemo(() => {
    ...
    const totalDeliveredItems = receipts.reduce((sum, r) => {
      ...
      } else if (r.status === 'Partially Delivered') {
        const sourceItems = r.hardwareItems || r.items;
  ...
  const getStatusBadge = (status: DeliveryReceipt['status']) => {
    switch (status) {
      case 'Delivered':
        return {
          label: 'Delivered',
          ...
          <table className="w-full text-left border-collapse min-w-[1050px]">
            ...
            <tbody className={`divide-y ${isDarkMode ? 'divide-slate-800' : 'divide-slate-100'}`}>
              {filteredReceipts.map((dr) => {
                const badge = getStatusBadge(dr.status);
                return (
                  <tr 
                    key={dr.id}
